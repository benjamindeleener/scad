...
def remove_outliers(img_centerline_unsmoothed, std=None,mu=None ):
    ...
    for i in range(z_max/2, z_max):
        if distance_phys[i] > 10:
            ...
            ind2 = np.argmax(centerline_unsmoothed[:, :, i+1])
            X2,Y2 = ind2sub(centerline_unsmoothed[:, :, i+1].shape,ind2)
            centerline_unsmoothed[X2,Y2,i+1] = 0
...
def symmetry_detector_right_left(data, dim):
    slice_p = np.squeeze(np.sum(data, 1))
...
class SCAD(Algorithm):
    ...
    def test_debug(self):
        ...
        normalised_symmetry = equalize_array_histogram(raw_symetry_data)
        enhanced_symmetry = np.power(normalised_symmetry, 2)
        spine_detect_data = np.multiply(minimum_path_data, enhanced_symmetry)
    ...
    def execute(self):
        ...
        if self.verbose:
            img.data = minimum_path_data
            img.change_orientation(raw_orientation)
            img.file_name += "_min_path"
            img.save()

        # multiply normalised symmetry data with the minimum path result
        spine_detect_data = np.multiply(minimum_path_data, np.power(equalize_array_histogram(raw_symetry_data),2))
        if self.verbose:
            img.data = spine_detect_data
            img.change_orientation('RPI')
            img.file_name = "symetry_weighted_min_path"
        ...
        # copy back centerline
        os.chdir('../')
        sct.tmp_copy_nifti(path_tmp + 'centerline.nii',self.input_image.path,self.input_image.file_name+'_centerline'+self.input_image.ext)
